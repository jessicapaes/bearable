...
<style>
    ...
    * {
    ...
    /* FIXED: Remove ALL bottom white space while preserving footer */
    ...
    /* Force body and html to not have extra space */
    html, body {
        margin: 0 !important;
        padding: 0 !important;
        overflow-x: hidden !important;
    ...
    /* FIXED: Allow content to flow naturally without clipping */
    .stApp {
    ...
    /* FIXED: Footer positioning and visibility */
    .custom-footer {
    ...
    /* Kill any bottom spacing */
    section.main > div {
        padding-bottom: 0 !important;
    }

    /* Remove Streamlit default footer only */
    footer[data-testid="stFooter"] {
        display: none !important;
    }

    /* FIXED: More aggressive targeting of white space containers */
    .main > div:last-child,
    ...
    /* FIXED: Target all Streamlit containers that might add white space */
    ...
    /* FIXED: Target the specific white space area */
    ...
    /* Make sure footer's parent containers don't add space */
    div:has(.custom-footer) {
        margin-bottom: 0 !important;
        padding-bottom: 0 !important;
    }
    ...
    /* Target specific modebar background only */
    .modebar-group {
        background: transparent !important;
    }

    /* Make the specific white rectangle transparent */
    svg.main-svg rect[style*="fill: rgb(255, 255, 255)"] {
...
# Add exit button if in demo mode - centered with Welcome section and tabs
if st.session_state.demo_mode:
    st.markdown("""
    <style>
    .exit-demo-wrapper {
    ...
    <div class="exit-demo-wrapper">
        <form action="" method="get" style="margin: 0;">
            <button type="submit" name="exit_demo" value="true" style="
    ...
    if st.query_params.get("exit_demo") == "true":
        st.session_state.demo_mode = False
        st.session_state.authenticated = False
        st.session_state.n1_df = pd.DataFrame()
...
with tab1:
    ...
    if not display_df.empty:
        ...
        st.markdown("<h3 style='text-align: center;'>📈 Your Health Metrics</h3>", unsafe_allow_html=True)

        # Center the gauges with more spacing
...
with tab4:
    ...
    st.markdown("""
    **Version:** 13.0 Final Edition (with numbered rankings & centered gauges)
...
<script>
// FIXED: More comprehensive white space removal
setTimeout(function() {
    // Remove all bottom padding and margins from Streamlit containers
    const containers = [
        '.main',
        '.block-container',
        '.stApp',
    ...
    // Target nested containers more aggressively
    ...
    // Remove any remaining white space by setting height to 0 on last containers
    const lastContainers = document.querySelectorAll('.stApp > div:last-child, .main > div:last-child');
    lastContainers.forEach(el => {
