...
with tab1:
    ...
    # Helper function to find therapy start
    def _find_therapy_start(df):
        """Find the first therapy start date and name in the dataframe"""
        if 'therapy_started' in df.columns:
            therapy_starts = df[df['therapy_started'].notna() & (df['therapy_started'] != '')]
            if not therapy_starts.empty:
                first_therapy = therapy_starts.iloc[0]
                return first_therapy['date'], first_therapy['therapy_started']
        return None, None
    ...
    if not display_df.empty:
        ...
        st.markdown("---")
        st.markdown("### 🔬 Causal Analysis: Therapy Effectiveness")

        # Find if there's a therapy intervention
        therapy_date, therapy_name = _find_therapy_start(display_df)

        if therapy_date:
            # DiD Analysis Section
            st.markdown(f"""
            <div style="background: linear-gradient(135deg, rgba(147, 51, 234, 0.1) 0%, rgba(236, 72, 153, 0.1) 100%);
                        padding: 1.5rem; border-radius: 15px; border-left: 5px solid #9333ea; margin: 1.5rem 0;">
                <h4 style="margin: 0 0 0.5rem 0; color: #9333ea; font-weight: 700;">
                    📊 Difference-in-Differences (DiD) Analysis
                </h4>
                <p style="margin: 0; color: #475569; line-height: 1.6;">
                    Measuring the <strong>causal effect</strong> of <strong>{therapy_name}</strong> on your pain levels
                    using a rigorous statistical method.
                </p>
            </div>
            """, unsafe_allow_html=True)
            ...
            # Convert therapy_date to pandas datetime for comparison
            therapy_date_pd = pd.to_datetime(therapy_date)

            # Create treatment indicator (1 after therapy starts, 0 before)
            df_analysis['post_treatment'] = (df_analysis['date'] >= therapy_date_pd).astype(int)

            # Calculate before and after means
            before_data = df_analysis[df_analysis['post_treatment'] == 0]
            after_data = df_analysis[df_analysis['post_treatment'] == 1]

            if len(before_data) >= 3 and len(after_data) >= 3:
                pain_before_mean = before_data['pain_score'].mean()
                pain_after_mean = after_data['pain_score'].mean()

                # Treatment effect (DiD estimator)
                treatment_effect = pain_after_mean - pain_before_mean
                percent_change = (treatment_effect / pain_before_mean * 100) if pain_before_mean > 0 else 0

                # Statistical significance test
                from scipy import stats
                t_stat, p_value = stats.ttest_ind(after_data['pain_score'], before_data['pain_score'])
                ...
                with col1:
                    ...
                    <div style="background: white; padding: 1.5rem; border-radius: 12px; text-align: center;
                                box-shadow: 0 4px 15px rgba(0,0,0,0.08); border: 2px solid #fee2e2;">
                        <div style="font-size: 0.9rem; color: #7f1d1d; font-weight: 600; margin-bottom: 0.5rem;">
                            📈 BEFORE {therapy_name.upper()}
                        </div>
                        <div style="font-size: 2.5rem; font-weight: 800; color: #dc2626; margin: 0.5rem 0;">
                            {pain_before_mean:.1f}
                        </div>
                        ...
                        <div style="font-size: 0.8rem; color: #94a3b8; margin-top: 0.5rem;">
                            n = {len(before_data)} days
                        </div>
                    </div>
                    """, unsafe_allow_html=True)

                with col2:
                    arrow = "↓" if treatment_effect < 0 else "↑"
                    color = "#10b981" if treatment_effect < 0 else "#ef4444"
                    bg_color = "#d1fae5" if treatment_effect < 0 else "#fee2e2"
                    label = "IMPROVEMENT" if treatment_effect < 0 else "INCREASE"

                    st.markdown(f"""
                    <div style="background: {bg_color}; padding: 1.5rem; border-radius: 12px; text-align: center;
                                box-shadow: 0 4px 15px rgba(0,0,0,0.08); border: 3px solid {color};">
                        <div style="font-size: 0.9rem; color: {color}; font-weight: 700; margin-bottom: 0.5rem;">
                            {arrow} TREATMENT EFFECT
                        </div>
                        <div style="font-size: 2.5rem; font-weight: 800; color: {color}; margin: 0.5rem 0;">
                            {abs(treatment_effect):.1f}
                        </div>
                        <div style="font-size: 0.85rem; color: #475569; font-weight: 600;">
                            {label}
                        </div>
                        <div style="font-size: 0.9rem; color: {color}; margin-top: 0.5rem; font-weight: 700;">
                            {abs(percent_change):.1f}% change
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                ...
                with col3:
                    st.markdown(f"""
                    <div style="background: white; padding: 1.5rem; border-radius: 12px; text-align: center;
                                box-shadow: 0 4px 15px rgba(0,0,0,0.08); border: 2px solid #d1fae5;">
                        <div style="font-size: 0.9rem; color: #065f46; font-weight: 600; margin-bottom: 0.5rem;">
                            📉 AFTER {therapy_name.upper()}
                        </div>
                        <div style="font-size: 2.5rem; font-weight: 800; color: #10b981; margin: 0.5rem 0;">
                            {pain_after_mean:.1f}
                        </div>
                        <div style="font-size: 0.85rem; color: #64748b;">
                            Average Pain Level
                        ...
                        <div style="font-size: 0.8rem; color: #94a3b8; margin-top: 0.5rem;">
                            n = {len(after_data)} days
                        </div>
                    </div>
                    """, unsafe_allow_html=True)

                # Statistical significance
                sig_badge = "✅ Statistically Significant" if is_significant else "⚠️ Not Statistically Significant"
                sig_color = "#10b981" if is_significant else "#f59e0b"
                sig_bg = "#d1fae5" if is_significant else "#fef3c7"

                st.markdown(f"""
                <div style="background: {sig_bg}; padding: 1rem 1.5rem; border-radius: 12px; margin-top: 1rem;
                            border-left: 5px solid {sig_color};">
                    <div style="display: flex; align-items: center; justify-content: space-between;">
                        <div>
                            <span style="font-weight: 700; color: {sig_color}; font-size: 1rem;">{sig_badge}</span>
                            <span style="color: #64748b; font-size: 0.9rem; margin-left: 1rem;">
                                p-value: {p_value:.4f} {'(p < 0.05)' if is_significant else '(p ≥ 0.05)'}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                # Interpretation
                if is_significant and treatment_effect < 0:
                    interpretation = f"""
                    <div style="background: white; padding: 1.5rem; border-radius: 12px; margin-top: 1rem; border: 2px solid #10b981;">
                        <h4 style="color: #10b981; margin: 0 0 0.75rem 0; font-size: 1.1rem;">
                            ✨ Key Findings
                        </h4>
                        <p style="margin: 0.5rem 0; color: #334155; line-height: 1.7;">
                            <strong style="color: #10b981;">Strong evidence</strong> that <strong>{therapy_name}</strong>
                            is <strong>reducing your pain</strong> by an average of <strong>{abs(treatment_effect):.1f} points</strong>
                            ({abs(percent_change):.1f}% improvement).
                        </p>
                        <p style="margin: 0.5rem 0; color: #334155; line-height: 1.7;">
                            📊 This result is <strong>statistically significant</strong> (p = {p_value:.4f}),
                            meaning it's <strong>unlikely due to chance</strong>.
                        </p>
                        <p style="margin: 0.5rem 0; color: #10b981; font-weight: 600;">
                            💡 Recommendation: Continue with {therapy_name} as it's showing measurable benefits!
                        </p>
                    </div>
                    """
                elif is_significant and treatment_effect > 0:
                    interpretation = f"""
                    <div style="background: white; padding: 1.5rem; border-radius: 12px; margin-top: 1rem; border: 2px solid #ef4444;">
                        <h4 style="color: #ef4444; margin: 0 0 0.75rem 0; font-size: 1.1rem;">
                            ⚠️ Key Findings
                        </h4>
                        <p style="margin: 0.5rem 0; color: #334155; line-height: 1.7;">
                            After starting <strong>{therapy_name}</strong>, pain levels have <strong>increased</strong>
                            by an average of <strong>{treatment_effect:.1f} points</strong> ({percent_change:.1f}% increase).
                        </p>
                        <p style="margin: 0.5rem 0; color: #334155; line-height: 1.7;">
                            📊 This change is <strong>statistically significant</strong> (p = {p_value:.4f}).
                        </p>
                        <p style="margin: 0.5rem 0; color: #ef4444; font-weight: 600;">
                    ...
                    interpretation = f"""
                    <div style="background: white; padding: 1.5rem; border-radius: 12px; margin-top: 1rem; border: 2px solid #94a3b8;">
                        <h4 style="color: #64748b; margin: 0 0 0.75rem 0; font-size: 1.1rem;">
                            📊 Key Findings
                        </h4>
                        <p style="margin: 0.5rem 0; color: #334155; line-height: 1.7;">
                            The observed change of <strong>{abs(treatment_effect):.1f} points</strong> after starting
                            <strong>{therapy_name}</strong> is <strong>not statistically significant</strong> (p = {p_value:.4f}).
                        </p>
                        <p style="margin: 0.5rem 0; color: #334155; line-height: 1.7;">
                            This could be due to:
                            <ul style="margin: 0.5rem 0 0 1.5rem; padding: 0;">
                                <li>Need more data (continue tracking!)</li>
                                <li>Natural fluctuations in pain levels</li>
                                <li>Therapy may need more time to show effects</li>
                            </ul>
                        </p>
                        <p style="margin: 0.5rem 0; color: #64748b; font-weight: 600;">
                            💡 Recommendation: Continue tracking for at least 30 more days for clearer results.
                        </p>
                    </div>
                    """
                ...
                # Methodology note
                with st.expander("📚 About This Analysis (Difference-in-Differences)"):
                    st.markdown("""
                    **What is Difference-in-Differences (DiD)?**

                    DiD is a causal inference method that compares the change in outcomes over time between
                    a treatment group (you, after starting therapy) and a control group (you, before therapy).

                    **How we calculated it:**
                    1. **Before Period**: Your pain levels before starting {therapy_name}
                    2. **After Period**: Your pain levels after starting {therapy_name}
                    3. **Treatment Effect**: Difference between After and Before averages
                    4. **Statistical Test**: T-test to determine if the difference is significant

                    **Assumptions:**
                    - Pain would have stayed similar without the therapy (parallel trends)
                    - No other major interventions started at the same time
                    - Consistent measurement of pain levels
                    ...
                    **Data used:**
                    - Before {therapy_name}: {len(before_data)} daily observations
                    - After {therapy_name}: {len(after_data)} daily observations
                    """.replace("{therapy_name}", therapy_name))
            else:
                st.info("📊 Need at least 3 days of data before and after starting therapy for causal analysis.")
        else:
            st.info("💡 Log a therapy start date in Daily Log to see causal analysis of its effectiveness!")
